...
/*                                                                        */
/**************************************************************************/
void  *_ux_utility_memory_allocate(unsigned long memory_alignment, unsigned long memory_cache_flag,
...
unsigned long       memory_for_alignment;
unsigned long       memory_removed_from_pool;
...
ALIGN_TYPE          int_memory_buffer;
    ...
    memory_size_requested =  (memory_size_requested +    UX_ALIGN_MIN) & (~(unsigned long)UX_ALIGN_MIN);
    memory_size_requested += (((unsigned long)sizeof(UX_MEMORY_BLOCK) + UX_ALIGN_MIN) & (~(unsigned long)UX_ALIGN_MIN)) - (unsigned long)sizeof(UX_MEMORY_BLOCK);
    ...
    memory_block =  _ux_utility_memory_free_block_best_get(memory_cache_flag, memory_size_requested + memory_for_alignment + (unsigned long)sizeof(UX_MEMORY_BLOCK));
    ...
    int_memory_buffer = (ALIGN_TYPE) ((unsigned char *) memory_block + sizeof(UX_MEMORY_BLOCK));
    ...
    if ((int_memory_buffer & (memory_alignment - 1)) == 0)
    {
        ...
        new_memory_block =  (UX_MEMORY_BLOCK *) ((unsigned char *) memory_block + sizeof(UX_MEMORY_BLOCK) + memory_size_requested);
        ...
        new_memory_block -> ux_memory_block_size =  memory_block -> ux_memory_block_size - memory_size_requested - (unsigned long)sizeof(UX_MEMORY_BLOCK);
        ...
        memory_removed_from_pool =  memory_block -> ux_memory_block_size + (unsigned long)sizeof(UX_MEMORY_BLOCK);
    }
    else
    {
        ...
        /* Align the buffer. The first thing we do is increment by the size of a
           ...
           least that.  */
        int_memory_buffer +=  (unsigned long)sizeof(UX_MEMORY_BLOCK);
        ...
        new_memory_block =  (UX_MEMORY_BLOCK *) (int_memory_buffer - (unsigned long)sizeof(UX_MEMORY_BLOCK));
        new_memory_block -> ux_memory_block_previous =  memory_block;
        new_memory_block -> ux_memory_block_next =  memory_block -> ux_memory_block_next;
        ...
        int_memory_buffer =  (ALIGN_TYPE) ((unsigned char *) memory_block + sizeof(UX_MEMORY_BLOCK));
        memory_block -> ux_memory_block_next =  new_memory_block;
        memory_block -> ux_memory_block_size =  (unsigned long) ((ALIGN_TYPE) new_memory_block - int_memory_buffer);

        /* Update the new memory block's size.  */
        new_memory_block -> ux_memory_block_size -=  (memory_block -> ux_memory_block_size + (unsigned long)sizeof(UX_MEMORY_BLOCK));
        ...
        if (leftover > sizeof(UX_MEMORY_BLOCK))
        {
            ...
            leftover_memory_block -> ux_memory_block_size =  leftover - (unsigned long)sizeof(UX_MEMORY_BLOCK);
        ...
        /* Declare how much memory we removed from the pool.  */
        memory_removed_from_pool =  new_memory_block -> ux_memory_block_size + (unsigned long)sizeof(UX_MEMORY_BLOCK);
    ...
    memory_buffer =  ((unsigned char *) memory_block) + sizeof(UX_MEMORY_BLOCK);
